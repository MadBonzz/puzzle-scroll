...
const coverage = JSON.parse(fs.readFileSync(path.join(implementationDir, 'coverage-report.json'), 'utf8'));
const negative = JSON.parse(fs.readFileSync(path.join(implementationDir, 'negative-controls.json'), 'utf8'));
...
const headers = ['ID', 'Production path', 'Test/oracle', 'Red evidence', 'Green evidence / seed', 'Negative control', 'Status', 'Limitation'];
const table = (header, values) => [
  '| ' + header.join(' | ') + ' |',
  '|' + header.map(() => '---').join('|') + '|',
  ...values.map((value) => '| ' + value.map((cell) => String(cell).replaceAll('|', '\\|').replaceAll('\n', ' ')).join(' | ') + ' |')
].join('\n');

const familyRows = coverage.families.map((family) => [
  'FAMILY-' + family.typeId,
  family.domain,
  family.disposition,
  family.approved ? 'catalog publication gate + audit evidence' : 'catalog exclusion gate',
  family.approved ? 'PASS' : 'PASS',
  family.approved
    ? 'Released v2; human timing provisional. ' + family.rationale
    : 'Withheld from practice/check/retry. ' + family.rationale
]);

const mutationPass = negative.controls.every((control) => control.caught);
const lines = [
  ...
  '- Negative controls: ' + (mutationPass ? '9/9 caught' : 'FAIL') + '; mutant failures are evidence, not shipped changes.',
  ...
  '## Phase gates',
  '',
  table(['Phase', 'Automated implementation', 'Release status', 'Reason'], [
  ...
  '',
  table(['ID', 'Fault', 'Result'], negative.controls.map((control) => [
    control.id,
    control.requirement,
    control.caught ? 'PASS (mutant rejected)' : 'FAIL (mutant survived)'
  ])),
  '',
  '## App-flow audit finding dispositions',
  '',
  table(['Audit ID', 'Status', 'Disposition', 'Evidence'], ux),
  '',
  ...
  '',
  table(['Requirement', 'Domain', 'Disposition', 'Gate/evidence', 'Status', 'Limitation'], familyRows),
  '',
...
fs.writeFileSync(path.join(implementationDir, 'verification-ledger.md'), lines.join('\n') + '\n');
